...
The simulation of the circuit shows that the calculated values are very close to the theoretical values. The load in the circuit is purely resistive, which means that the output current is in phase with the output voltage. The full-wave rectification of the AC signal results in an output DC signal with twice the frequency of the input signal, which can be useful in certain applications that require high-frequency DC power.
\pagebreak
