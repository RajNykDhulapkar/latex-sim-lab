...
Firing pulses are required to start the output voltage in the circuit. Once triggered, the circuit acts as a full bridge uncontrolled rectifier. The simulated values show a good match with the theoretical values. The load is resistive, so the output current and voltage are in phase. The output DC signal has a frequency that is twice the frequency of the input AC signal.
